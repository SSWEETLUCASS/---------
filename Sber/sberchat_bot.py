...
def process_message(message: UpdateMessage) -> None:
    user_id = message.sender.uid
    msg = message.message.text_message.text.strip()
    peer = message.peer

    # Приветственное меню с кнопками (отправляется при любом сообщении)
    # Можно поменять логику, чтобы это было по команде /start или если пользователь в состоянии ожидания
    if user_id not in user_states:
        user_states[user_id] = {}

    # Если бот ожидает ответ от пользователя (состояния), обрабатываем их
    ...
    if user_id in user_states and user_states[user_id].get("mode") == "choose":
        if msg.lower() in ("шаблон", "по шаблону"):
            ...
            bot.messaging.send_message(peer, f"1⃣ {TEMPLATE_FIELDS[0]}:")
        elif msg.lower() in ("свободно", "свободная форма"):
            user_states[user_id]["mode"] = "freeform"
            bot.messaging.send_message(peer, "📝 Опишите инициативу в свободной форме:")
        else:
            bot.messaging.send_message(peer, "❓ Пожалуйста, выберите: шаблон или свободно.")
        return

    if user_states.get(user_id, {}).get("mode") == "freeform":
        idea_text = msg
        bot.messaging.send_message(peer, "🔍 Проверяю через GigaChat...")
        ...
        if is_unique:
            bot.messaging.send_message(peer, "✅ Идея уникальна!")
            if parsed_data:
                word_path, excel_path = generate_files(parsed_data)
                with open(word_path, "rb") as doc_file:
                    bot.messaging.send_file(peer, doc_file.read(), word_path)
                with open(excel_path, "rb") as excel_file:
                    bot.messaging.send_file(peer, excel_file.read(), excel_path)
                os.remove(word_path)
                os.remove(excel_path)
            else:
                bot.messaging.send_message(peer, "⚠️ Не удалось распознать поля для генерации шаблонов.")
        else:
            bot.messaging.send_message(peer, "⚠️ Идея не уникальна или неполна.")
    ...
    if user_states.get(user_id, {}).get("mode") == "template":
        ...
        if step is not None and step < len(TEMPLATE_FIELDS):
            ...
            if state["step"] < len(TEMPLATE_FIELDS):
                ...
                bot.messaging.send_message(peer, f"{state['step'] + 1}⃣ {next_field}:")
            else:
                bot.messaging.send_message(peer, "🔍 Проверяю через GigaChat...")
                data = {
                ...
                bot.messaging.send_message(peer, f"🤖 Ответ GigaChat:\n\n{result}")
                if is_unique:
                    bot.messaging.send_message(peer, "✅ Идея уникальна! Генерирую документы...")
    ...
    if msg.lower() == "все агенты":
        try:
            with open("agents.xlsx", "rb") as f:
                bot.messaging.send_file(peer, f.read(), "agents.xlsx")
        except Exception as e:
            bot.messaging.send_message(peer, f"❌ Не удалось отправить файл: {e}")
        return

    if msg.lower() == "искать по названию":
        agent_query_state[user_id] = True
        bot.messaging.send_message(peer, "🔍 Введи название агента:")
        return
    ...
    if agent_query_state.get(user_id):
        ...
        try:
            ...
            if not results:
                bot.messaging.send_message(peer, "❌ Агент не найден.")
            else:
                for r in results:
            ...
            bot.messaging.send_message(peer, f"⚠️ Ошибка при поиске: {e}")
        return

    if msg == "Кто поможет?💬":
        bot.messaging.send_message(peer, "🧑‍💻 Пока эта функция в разработке.")
        return

    if msg == "Поддержка📝":
        bot.messaging.send_message(peer, "✉️ Напиши нам")
        return

    bot.messaging.send_message(peer, "🤖 Я вас не понял. Пожалуйста, выбери действие из меню!")
...
def main():
    ...
    bot.messaging.message_handler([
        MessageHandler(process_message, MessageContentType.TEXT_MESSAGE),
    ])
