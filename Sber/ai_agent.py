...
def check_general_message_with_gigachat(msg: str, user_id: int = None) -> tuple[str, str | None]:
    ...
    try:
        ...
        Проанализируй сообщение(может быть нестандартным, найди самое близкое по значению) и определи, что подходит:
        ...
        5. Если хочет получить список аи-агентов, то CMD:ai_agent

        6. Если пользователь хочет консультацию → CMD:consultation

        7. Если пользователь хочет узнать про аи агента или его владельца, то CMD:search_owners
        
        8. Если ничего из выше перечисленного не подходит, дай полезный ответ по смыслу.

        Отвечай ТОЛЬКО на русском языке, без дополнительной технической информации 1 сообщением. До 4000 слов.и не забудь смайлики.
        """
...
def generate_idea_suggestions(query: str = "") -> str:
    ...
    try:
        ...
        Отвечай ТОЛЬКО на русском языке, структурированно и понятно. и не забудь смайлики.
        """
