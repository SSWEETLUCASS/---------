...
def check_general_message_with_gigachat(msg: str, user_id: int = None) -> tuple[str, bool, str | None]:
    ...
    try:
        ...
        4. Хочет ли пользователь выполнить команду:
           ...
           - рассказать про конкретного агента по имени или краткому названию
             (например: "расскажи про агента Иван", "информация об агенте test", "что за агент X") → CMD:agent_info
