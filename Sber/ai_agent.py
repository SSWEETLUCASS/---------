...
class GigaChatProcessor:
    ...
    def _generate_general_message_prompt(self, msg: str, conversation_context: str) -> str:
        ...
        2. Первичное описание идеи → CMD:idea
        3. Уточнение по идее или её развитие → остаёмся в процессе (без CMD)
        4. Жалобы на проблемы с ботом → CMD:help
        5. Запрос списка агентов → CMD:ai_agent
        6. Консультация (ссылки, рекомендации) → CMD:consultation
        7. Вопросы про владельцев → CMD:search_owners
        8. Иначе — полезный ответ без команды


        Особенности ответа:
        - Если заполнение по информации ≥70%, то считаем, что можно сразу переходить к заполнению.  
        - Если <70%, уточняй недостающие поля.  
