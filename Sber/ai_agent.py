...
def calculate_work_cost_interactive(parsed_data: dict, user_id: int = None, answers: dict = None) -> tuple[str, dict]:
    """
    Интерактивный расчет стоимости работы по инициативе с уточняющими вопросами.
    
    Args:
        parsed_data: Данные об инициативе
        user_id: ID пользователя для сохранения в память
        answers: Словарь с ответами пользователя на вопросы
        
    Returns:
        tuple: (текст ответа, словарь с вопросами или None если расчет завершен)
    """
    try:
        # Если ответов нет, генерируем вопросы
        if not answers:
            return generate_cost_questions(parsed_data)
        
        # Если ответы есть, делаем расчет
        return calculate_final_cost(parsed_data, answers, user_id)
        
    except Exception as e:
        logging.error(f"Ошибка в интерактивном расчете стоимости: {e}")
